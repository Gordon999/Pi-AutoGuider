...
hist =                0 # equalise / 2x2 binning , 0 = OFF, 1 = stretch histogram, 2 = 2x2 binning
...
usb_min_co   = 0
usb_max_co   = 63
exposure     = 255 # sets USB camera initial exposure
usb_min_ex   = 0
usb_max_ex   = 255
...
button(b1x, 3, b1y, 2, bw, 1, bh, hist)
...
if use_Pi_Cam:
   ...
   button(b2x, 5, b2y, 5, bw, 1, bh, Auto_Gain)
...
oldgamma,oldexposure,oldgain,oldminc,oldnscale, oldescale, oldsscale, oldwscale, oldcrop, oldauto_g, oldInterval, oldlog, oldrgbw, oldthreshold, oldgraph, oldthres, oldauto_i, oldplot, oldauto_win, oldauto_t, oldzoom, oldrpibr, oldrpico, oldrpiss, oldrpiexno, oldrpiISO, oldrpiev, oldcls, oldcon_cap, oldphoto, oldptime, oldpcount, oldrpired, oldrpiblue, oldhist, oldnr, olddecN, olddecS, oldnsi, oldewi = gamma,exposure, gain,minc,nscale, escale, sscale, wscale, crop, auto_g, Interval, log, rgbw, threshold, graph, thres, auto_i, plot, auto_win, auto_t, zoom, rpibr, rpico, rpiss, rpiexno, rpiISO, rpiev, cls, con_cap, photo, ptime, pcount, rpired, rpiblue, hist, nr, decN, decS, nsi, ewi
...
# calculate min and max values
   ...
   mintot = 0
   tval = 0
   while tval < sum(arp) * 0.0001 and count < 299:
      tval += arp[count]
      count +=1
      mintot = count
   count =  2
   noitot = 0
   tval = 0
   while tval < sum(arp) * 0.6 and count < 299:
      tval += arp[count]
      count +=1
      noitot = count
   count =  255
...
#display graph if enabled
   if graph > 0:
      ...
      for count in range (0, 256):
         if hist == 1 and count != maxtot and count != mintot and count != upnoi and count != pcont:
            val = arp[count+1]
            ...
            img += byte
         if hist != 1 and count != maxtot and count != noitot and count != pcont:
            val = arp[count+1]
            val2 = int(30*math.log10(val + 10) - 29)
            val2 = min(val2, 49)
            val2 = max(val2, 1)
         ...
         if count == int(pcont):
            img += redline[0:pov*3]
         elif count == int(noitot + a_thr_limit) and esc1 > 0 and auto_t == 1 and hist != 1:
            img += yelline[0:pov*3]
         elif count == int(noitot + m_thr_limit) and esc1 > 0 and auto_t == 0 and hist != 1:
            img += yelline[0:pov*3]
         elif count == int(upnoi + a_thr_limit) and esc1 > 0 and auto_t == 1 and hist == 1:
            img += yelline[0:pov*3]
         elif count == int(upnoi + m_thr_limit) and esc1 > 0 and auto_t == 0 and hist == 1:
            img += yelline[0:pov*3]
...
# read mouse or Keyboard
      ...
      if xouse < Disp_Width and youse < Disp_Height:
         ...
         if buttonx[mouse_button] != 0 and menu == 0 and store == 0 and calibrate == 0:
            ...
            if graph == 0 or (graph > 0 and (xouse < pox or youse > poy + 258)):
               store = 1
   elif buttonx[mouse_button] != 0 and menu == 0 and store == 1:
      pos = pygame.mouse.get_pos()
      mousexx = pos[0]
      mouseyy = pos[1]
   ...
   if buttonx[0] != 0 or mousepress == 1 and store == 0 and calibrate == 0:
      ...
      if z == 4:         
         ...
         p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         change = 1
      elif z == 65 and not use_Pi_Cam and camera_connected and esc1 == 0:
         gamma -=1
         gamma = max(gamma,usb_min_ga)
         rpistr = "v4l2-ctl -c gamma=" + str(gamma) + " -d " + str(dve)
         p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         change = 1
      elif z == 75 and not use_Pi_Cam and camera_connected and esc1 == 0:
         gamma +=1
            ...
            change = 1
             ...
             if offset3 != 0 :
                ...
                while offtry < offmoves:
                   ...
                   #print (offtry,offset3,offset4,offset3a,offset4a,offset5,offset6)
                   if (offset6 > 0 and offset6 >= h/2 - Disp_Height/2) or (offset6 < 0 and offset6 <= Disp_Height/2 - h/2) or (offset5 > 0 and offset5 >= w/2 - Disp_Width/2) or (offset5 < 0 and offset5 <= Disp_Width/2 - w/2):
                      offset6 -= int(offset4a/10)
                      offset5 -= int(offset3a/10)
                      offtry = offmoves
             ...
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             change = 1
          elif z == 65 and not use_Pi_Cam and camera_connected and esc1 == 0 and press == 0:
             gamma -=1
             gamma = max(gamma,usb_min_ga)
             rpistr = "v4l2-ctl -c gamma=" + str(gamma) + " -d " + str(dve)
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             change = 1
          elif z == 75 and not use_Pi_Cam and camera_connected and esc1 == 0 and press == 0:
             gamma +=1
             gamma = min(gamma,usb_max_ga)
             rpistr = "v4l2-ctl -c gamma=" + str(gamma) + " -d " + str(dve)
             #print (rpistr)
             ...
             if use_Pi_Cam:
                rpiredx = Decimal(rpired)/Decimal(100)
             if not use_Pi_Cam and camera_connected and press == 0:
                rpiredx = max(rpired, usb_min_rx)
                rpistr = "v4l2-ctl -c red_balance=" + str(rpiredx) + " -d " + str(dve)
                p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             restart = 1
             ...
             if use_Pi_Cam and camera_connected:
                os.killpg(p.pid, signal.SIGTERM)
             if (not use_Pi_Cam and zoom < usb_max_res + 2) or (use_Pi_Cam and Pi_Cam == 2 and zoom < 9) or (use_Pi_Cam and Pi_Cam == 1 and zoom < 7):
                zoom += 1
                if nr > 0:
                   mxq = []
                   mxp = []
