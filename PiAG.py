...
power_down =          0 # set = 1 to power down the Pi when you exit the script, clicking Esc twice.
                        # To stop the script without existing when set, click Esc then TELESCOPE.
#=======================================================================================================
# CONNECTION SETTINGS
...
Cwindow =             1 # Enable circular window
...
#===================================================================================
#Telescope control GPIO 'physical' pins
N_OP =               22 #22
S_OP =               18 #18
E_OP =               24 #24
W_OP =               16 #16
...
if camera_connected and not use_Pi_Cam:
   pygame.camera.init()
   cam = pygame.camera.Camera("/dev/video0", (rpiwidth[usb_max_res],rpiheight[usb_max_res]))
   print (rpiwidth[usb_max_res],rpiheight[usb_max_res])
   cam.start()
...
# take picture
  # webcam
   ...
   if camera_connected and use_Pi_Cam:
      ...
      rpistopEW = 0
      mkey =      0
      # check next camera image is ready, and wait if not, checking for mousekey press
      ...
      if maxwin != crop:
         if menu == 0:
            if Cwindow == 0 :
               ...
               pygame.draw.circle(windowSurfaceObj,  redColor, (int((w2-c1)+int(crop/2)),int((h2-c2)+int(crop/2))), int(11),1)
         else:
            if Cwindow == 0 :            
...
# read mouse or Keyboard
   ...
   for event in pygame.event.get():
       if event.type == QUIT:
          ...
          if z == 115 or kz == 304 or kz == 303:
             ...
             if esc1 == 0:
                keys(0,"Esc",                      fs,       2,        b3x,         bw,   2,     b3y, bh, 2, 2, 0)
             if esc1 > 0:
                if use_Pi_Cam and camera_connected:
                   os.killpg(p.pid, signal.SIGTERM)
                #if use_Pi_Cam == 0  and camera_connected:
                #   cam.stop()
                if serial_connected:
                   lx200(':Q#', ':Q#', decN, decS)
                if use_RPiGPIO or use_Seeed or use_PiFaceRP:
             ...
             if auto_c > 1:
                ...
                auto_win = 0
                crop = ocrop
             keys(0,"cen",  fs, auto_c, b3x, bw, 1, b3y, bh, 3, 0, 1)
             keys(0," tre", fs, auto_c, b3x, bw, 1, b3y, bh, 3, 2, 1)
   ...
   if change :
      ...
      if oldcrop != crop:
         ...
         oldcrop = crop
      if oldrpibr != rpibr and use_Pi_Cam == 1:
