...
# THEN ON THE GUI OPEN LXTerminal AND ENTER 'sudo idle3' for PYTHON 3
...
# Add @sudo /usr/bin/python3 /home/pi/PiAG.py to the end of ~/.config/lxsession/LXDE-pi/autostart.
...
Webcam =              2 # 0 = Philips, 1 = Logitech, 2 = Pi Camera using v4l2
                        # if using Webcam = 2 then install imutils (sudo pip3 install imutils)
usb_max_res =         6 # usb_max_res - Webcam max available resolution, depends on your Webcam
...
use_RPiGPIO =         0 # if using the RPi.GPIO on the Pi set use_RPiGPIO = 1, not required if only using the DSLR GPIO O/P (photoon = 1).
...
if use_config > 0 and use_config < 4:
   ...
   if os.path.exists(deffile + ".txt"):
       with open(deffile + ".txt", "r") as file:
           ...
           rpired =  int(inputx[81:85])
           rpiblue = int(inputx[85:89])
           if use_Pi_Cam:
              ...
              rpiredx = rpired
              rpibluex = rpiblue
           pcount =  int(inputx[89:92])
           ptime =   int(inputx[92:94])
           minc =    Decimal(int(inputx[146:149]))/Decimal(10)
           a_thr_limit = int(inputx[149:152])
           m_thr_limit = int(inputx[152:155])
           calibrate_time = int(inputx[155:158])
           Cang = int(inputx[158:162])
           if Cang > 9000:
              Cang = 9000 - Cang
           exposure = int(inputx[162:166])
           gain = int(inputx[166:169])
           gamma = int(inputx[169:172])
           Auto_Gain = int(inputx[172:175])
           sharpness  = int(inputx[175:178])
           saturation = int(inputx[178:181])
           color_temp = int(inputx[181:184]) * 10
       if Cwindow:
...
if not use_Pi_Cam and camera_connected:
   ...
   if Webcam == 0 : # Philips Webcam initialisation
      ...
      p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
   elif Webcam == 2 : # Pi Cam as Webcam initialisation
      ...
      p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
      rpistr = "v4l2-ctl -c iso_sensitivity=4" + " -d " + str(dve)
      p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
...
# read mouse or Keyboard
   ...
   if buttonx[0] != 0 or mousepress == 1 and store == 0 and calibrate == 0:
      ...
      if z == 161  and zoom > Image_window:
         ...
         if (use_Pi_Cam or (not use_Pi_Cam and (Webcam == 0 or Webcam == 2))) and camera_connected:
            if Webcam == 2:
               rpired -=20
            else:
               rpired -=1
         ...
         if use_Pi_Cam and camera_connected:
            rpiredx = Decimal(rpired)/Decimal(100)
         elif not use_Pi_Cam and camera_connected and (Webcam == 0 or Webcam == 2):
         ...
         change = 1
      elif z == 64 and not use_Pi_Cam and camera_connected and Webcam == 2:
         scene_mode = 0
         ...
         if Auto_Gain == 0:
             keys(0,str(exposure),           fs,       3,        b2x,         bw,   1,     b2y, bh, 4, 3, 1)
         change = 1
      elif z == 74 and not use_Pi_Cam and camera_connected and Webcam == 2:
         scene_mode = 8
         rpistr = "v4l2-ctl -c scene_mode=" + str(scene_mode) + " -d " + str(dve)
         p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         keys(0,str(scene_mode),      fs,       3,        b2x,         bw,   1,     b2y, bh, 5, 3, 0)
         ...
         change = 1
      elif z == 65 and not use_Pi_Cam and camera_connected and esc1 == 0 and Webcam == 2:
         rpiev -=1
         rpiev = max(rpiev,usb_min_ev)
         change = 1
      elif z == 75 and not use_Pi_Cam and camera_connected and esc1 == 0 and Webcam == 2:
         rpiev +=1
          ...
          if z == 115 or kz == 304 or kz == 303:
            ...
            change = 1
             ...
             if camera_connected == 0:
                posx =  Disp_Width/2
                posy = Disp_Height/2
          elif z == 63 and not use_Pi_Cam and camera_connected and press == 0 and Auto_Gain == 0 and ( Webcam == 2 and scene_mode == 0):
             if Webcam == 0 :
                exposure -=1
             else:
                exposure -=10
             ...
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             change = 1
          elif z == 73 and not use_Pi_Cam and camera_connected and press == 0 and Auto_Gain == 0 and ( Webcam == 2 and scene_mode == 0):
             if Webcam == 0:
                exposure +=1
             else:
                exposure +=10
             ...
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             change = 1
          elif z == 75 and not use_Pi_Cam and camera_connected and esc1 == 0 and press == 0 and Webcam == 2:
             rpiev +=1
             rpiev = min(rpiev,usb_max_ev)
             change = 1
          elif z == 65 and not use_Pi_Cam and camera_connected and esc1 == 0 and press == 0 and Webcam == 2:
             rpiev -=1
             rpiev = max(rpiev,usb_min_ev)
             change = 1
             ...
             if use_Pi_Cam and camera_connected:
                rpiredx = Decimal(rpired)/Decimal(100)
             elif not use_Pi_Cam and camera_connected and press == 0 and (Webcam == 0 or Webcam == 2):
                rpiredx = max(rpired, usb_min_rx)
                rpistr = "v4l2-ctl -c red_balance=" + str(rpiredx) + " -d " + str(dve)
                p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             ...
             if use_Pi_Cam or (use_Pi_Cam == 0 and Webcam < 2):
                timp += "0"
   ...
   if change :
      ...
      if oldInterval != Interval:
         keys(0,str(int(Interval)), fs, 3, b1x, bw, 5, b1y, bh, 5, 3, 1)
      if oldrpired != rpired :
         if use_Pi_Cam == 1 or (not use_Pi_Cam and (Webcam == 0 or Webcam == 2)):
            keys(0,str(rpiredx), fs, 3, b1x, bw, 1, b1y, bh, 5, 3, 1)
         if not use_Pi_Cam  and (Webcam == 0 or Webcam == 2)  and camera_connected:
            rpistr = "v4l2-ctl -c red_balance=" + str(rpired) + " -d " + str(dve)
            p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         oldrpired = rpired
      ...
      if oldrpiev != rpiev and camera_connected and use_Pi_Cam:
         keys(0,str(int(rpiev)), fs, 3, b2x, bw, 1, b2y, bh, 4, 3, 1)
         oldrpiev = rpiev
      if oldrpiev != rpiev and camera_connected and not use_Pi_Cam and Webcam == 2:
         keys(0,str(rpiev),            fs,       3,        b2x,         bw,   1,     b2y, bh, 6, 3, 1)
         rpistr = "v4l2-ctl -c auto_exposure_bias=" + str(rpiev + 12) + " -d " + str(dve)
         p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         oldrpiev = rpiev
      ...
      if oldexposure != exposure and not use_Pi_Cam:
         ...
         if Webcam == 0 and camera_connected:
            ...
            rpistr = "v4l2-ctl -c exposure_absolute=" + str(exposure) + " -d " + str(dve)
         elif Webcam == 2 and camera_connected:
            rpistr = "v4l2-ctl -c exposure_time_absolute=" + str(exposure) + " -d " + str(dve)
         p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
