...
# to the end of ~/.config/lxsession/LXDE-pi/autostart. Make the file executable with sudo chmod +x PiAG.py
...
Webcam =              1 # 0 = Philips, 1 = Logitech
usb_max_res =         4 # usb_max_res - webcam max available resolution, depends on your webcam
...
Pi_LCD_Display =      1 # Set = 1 if using the Pi 7" LCD Display v1.1, will then set brightness for Day / Night
...
nsi =                 1 # nsi - Invert North<>South invert = 1, non-invert = 0
...
auto_t =              1 # auto_t - auto threshold, on = 1, off = 0
...
brightness   = 107 # sets USB camera initial brightness
contrast     = 32  # sets USB camera initial contrast
...
usb_min_br   = 0
if Webcam == 0:
   usb_max_br   = 127 # Philips
else:
   usb_max_br   = 255 # Logitech
#contrast
usb_min_co   = 0
if Webcam == 0:
   usb_max_co   = 63 # Philips
else:
   usb_max_co   = 255 # Logitech
#exposure
usb_min_ex   = 0
if Webcam == 0:
   usb_max_ex   = 255 # Philips
else:
   usb_max_ex   = 1000 # Logitech
#gain
usb_min_gn   = 0
if Webcam == 0:
   usb_max_gn   = 63  # Philips
else:
   usb_max_gn   = 255  # Logitech

# Following USB settings only for Philips...
#============================================
...
usb_min_bx   = 0
usb_max_bx   = 255
auto_contour = 0   # 0 switches it to manual 0-1
contour      = 0   # sets to min, range 0-63
dnr          = 0   # sets to min, range 0-3
backlight    = 0   # sets it OFF 0-1

# Following USB settings only for Logitech...
#============================================
sharpness    = 128   # sets USB camera initial sharpness
...
if use_config > 0 and use_config < 4:
   ...
   if os.path.exists(deffile + ".txt"):
      ...
      if len(inputx) > 159:
         calibrate_time = int(inputx[153:156])
         Cang =           int(inputx[156:160])
         if Cang > 9000:
            Cang = 9000 - Cang
      if len(inputx) > 168:
         exposure = int(inputx[160:163])
         gain = int(inputx[163:166])
         gamma = int(inputx[166:169])
...
oldsaturation,oldcolor_temp,oldsharpness,oldAuto_Gain,oldgamma,oldexposure,oldgain,oldminc,oldnscale, oldescale, oldsscale, oldwscale, oldcrop, oldauto_g, oldInterval, oldlog, oldrgbw, oldthreshold, oldgraph, oldthres, oldauto_i, oldplot, oldauto_win, oldauto_t, oldzoom, oldrpibr, oldrpico, oldrpiss, oldrpiexno, oldrpiISO, oldrpiev, oldcls, oldcon_cap, oldphoto, oldptime, oldpcount, oldrpired, oldrpiblue, oldhist, oldnr, olddecN, olddecS, oldnsi, oldewi = saturation,color_temp,sharpness,Auto_Gain,gamma,exposure, gain,minc,nscale, escale, sscale, wscale, crop, auto_g, Interval, log, rgbw, threshold, graph, thres, auto_i, plot, auto_win, auto_t, zoom, rpibr, rpico, rpiss, rpiexno, rpiISO, rpiev, cls, con_cap, photo, ptime, pcount, rpired, rpiblue, hist, nr, decN, decS, nsi, ewi
...
# take picture
  # webcam
   if camera_connected and not use_Pi_Cam:
      image = cam.get_image()
      pygame.image.save(image, '/run/shm/i.jpg')
      if calibrate == 1:
...
#display graph if enabled
   ...
   if calibrate == 1 and cal_count == 0:
      newscale = nscale
      oauto_g = auto_g
      auto_g = 0
      oauto_win = auto_win
   ...
   elif calibrate == 1 and cal_count < samples + 1:
      if samples > 0:
         cal_count +=1
   ...
   elif calibrate == 1 and (cal_count == samples + 2 or cal_count == samples + 3)  :
      if ttot < 6:
         calibrate = 0
         cal_count = -1
         crop = ocrop
      ...
      if calibrate == 1 and cal_count == samples + 2:
         keys(0,str(int(threshold)),fs,3,b1x,bw,5,b1y, bh, 4, 3, 1)
         zt = 0
         scale_t = 1
         while zt <= zoom:
         ...
         pygame.display.update()
      if calibrate == 1 and cal_count == samples + 3:
         tim = cal_time
         oacorrect = acorrect
         obcorrect = bcorrect
      if calibrate == 1 and serial_connected:
         ...
         if nsi == 0:
            ...
            button(b3x, 2, b3y, 2, bw, 1, bh, 0,0)
            keys(0,Dkey[0], fs-1, 6, b3x, bw, 1, b3y, bh, 2, 2, 1)
      if calibrate == 1 and (use_RPiGPIO or use_Seeed or use_PiFaceRP):
         if nsi == 0:
            button(b3x, 2, b3y, 4, bw, 1, bh, 1,0)
            keys(0,Dkey[3], fs-1, 1, b3x, bw, 1, b3y, bh, 4, 2, 1)
            DEVICE_ADDRESS, DEVICE_REG_MODE1, DEVICE_REG_DATA = R_OFF(0, DEVICE_ADDRESS, DEVICE_REG_MODE1, DEVICE_REG_DATA)
   ...
   elif calibrate == 1 and cal_count < (samples * 2) + 4:
      if samples > 0:
         cal_count +=1
   ...
   elif calibrate == 1 and (cal_count == (samples * 2) + 5 or cal_count == (samples * 2) + 6):
   ...
   # calibrate RA
   ...
   elif (calibrate == 1 and cal_count > ((samples *2) + 6) and cal_count <  (samples * 3) + 7) or (calibrate == 1 and nr == 0 and cal_count == (samples *3) + 6):
      if samples > 0:
         cal_count +=1

   elif calibrate == 1 and cal_count == (samples * 3) + 7:
      offset3 = offset3 + int(bcorrect/100)
      offset4 = offset4 - int(acorrect/100)
      cal_count +=1
      
   elif calibrate == 1 and (cal_count == (samples *3) + 8 or cal_count == (samples *3) + 9):
      ...
      cal_count +=1
      time.sleep((1))
   elif calibrate == 1 and cal_count > ((samples * 3)  + 9) and cal_count <  (samples * 4) + 10 or (calibrate == 1 and nr == 0 and cal_count == (samples *3) + 9):
      if samples > 0:
         cal_count +=1

      
   elif calibrate == 1 and cal_count == (samples * 4) + 10:
      ccorrect = acorrect - oacorrect
      dcorrect = bcorrect - obcorrect
      if (abs(ccorrect)) > move_limit or (abs(dcorrect)) > move_limit:
         if dcorrect == 0:
      ...
      if use_Pi_Cam and camera_connected:
         ...
         if not rpiISO:
            keys(0,'auto',               fs,   3,        b2x,         bw,   1,     b2y, bh, 6, 3, 1)
      if not use_Pi_Cam and camera_connected and Webcam == 0:
         button(b2x, 1, b2y, 6, bw, 2, bh, 0,1)
         if Webcam == 0:
            keys(0,"Gamma",              fs,   6,        b2x,         bw,   0,     b2y, bh, 6, 0, 1)
            keys(0," -",                 fs,   6,        b2x,         bw,   0,     b2y, bh, 6, 4, 1)
   ...
   if calibrate == 0 and (no_star == 1 or no_move == 1):
      no_star = 0
      no_move = 0
      time.sleep(2)
...
# read mouse or Keyboard
   ...
   if buttonx[mouse_button] != 0 and menu == 0 and store == 0 and calibrate == 0:
   ...
   if buttonx[0] != 0 or mousepress == 1 and store == 0 and calibrate == 0:
      ...
      if z == 4:
         if use_Pi_Cam or (not use_Pi_Cam and Webcam == 0):
            rpired -=1
         else:
            saturation -=1
         if use_Pi_Cam:
         ...
         if use_Pi_Cam and camera_connected:
            rpiredx = Decimal(rpired)/Decimal(100)
         if not use_Pi_Cam and camera_connected and Webcam == 0 and camera_connected:
            rpiredx = min(rpired, usb_max_rx)
            rpistr = "v4l2-ctl -c red_balance=" + str(rpiredx) + " -d " + str(dve)
            p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         if not use_Pi_Cam and camera_connected and Webcam == 1 and camera_connected:
         ...
         if not use_Pi_Cam and camera_connected:
            ...
            rpico = min(rpico, 100)
         change = 1
      elif z == 63 and not use_Pi_Cam and camera_connected and Auto_Gain == 0:
         if Webcam == 0:
            exposure -=1
         else:
            exposure -=10
         ...
         change = 1
      elif z == 73 and not use_Pi_Cam and camera_connected and Auto_Gain == 0:
         if Webcam == 0:
            exposure +=1
         else:
            exposure +=10
         ...
         change = 1
      elif z == 64 and not use_Pi_Cam and camera_connected and Auto_Gain == 0:
         gain -=1
         gain = max(gain,usb_min_gn)
         change = 1
      elif z == 74 and not use_Pi_Cam and camera_connected and Auto_Gain == 0:
         gain +=1
   ...
   for event in pygame.event.get():
       ...
       elif (event.type == MOUSEBUTTONUP or event.type == KEYDOWN) and calibrate == 0 :
          start = time.time()
          xcount =     0
          ycount =     0
          ymax =       0
          ...
          if z == 115 or kz == 304 or kz == 303:
            ...
            change = 1
             ...
             if camera_connected == 0:
                posx =  Disp_Width/2
                posy = Disp_Height/2
          elif z == 63 and not use_Pi_Cam and camera_connected and Auto_Gain == 0 and press == 0:
             if Webcam == 0:
                exposure -=1
             else:
                exposure -=10
             ...
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             change = 1
          elif z == 73 and not use_Pi_Cam and camera_connected and Auto_Gain == 0 and press == 0:
             if Webcam == 0:
                exposure +=1
             else:
                exposure +=10
             ...
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             change = 1
          elif z == 64 and not use_Pi_Cam and camera_connected and Auto_Gain == 0 and press == 0:
             gain -=1
             gain = max(gain,usb_min_gn)
             rpistr = "v4l2-ctl -c gain=" + str(gain) + " -d " + str(dve)
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             change = 1
          elif z == 74 and not use_Pi_Cam and camera_connected and Auto_Gain == 0 and press == 0:
             gain +=1
             gain = min(gain,usb_max_gn)
             rpistr = "v4l2-ctl -c gain=" + str(gain) + " -d " + str(dve)
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             ...
             if use_Pi_Cam and camera_connected:
                rpiredx = Decimal(rpired)/Decimal(100)
             if not use_Pi_Cam and camera_connected and press == 0 and Webcam == 0:
                rpiredx = max(rpired, usb_min_rx)
                rpistr = "v4l2-ctl -c red_balance=" + str(rpiredx) + " -d " + str(dve)
                p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             if not use_Pi_Cam and camera_connected and press == 0 and Webcam == 1:
                saturation = max(saturation, usb_min_sa)
                rpistr = "v4l2-ctl -c saturation=" + str(saturation) + " -d " + str(dve)
                p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             restart = 1
             ...
             if use_Pi_Cam and camera_connected:
                os.killpg(p.pid, signal.SIGTERM)
             if (not use_Pi_Cam and Webcam == 0 and zoom < usb_max_res + 2) or (not use_Pi_Cam and Webcam == 1 and zoom < usb_max_res + 1) or (use_Pi_Cam and Pi_Cam == 2 and zoom < 9) or (use_Pi_Cam and Pi_Cam == 1 and zoom < 7):
                zoom += 1
                if nr > 0:
                   mxq = []
                   mxp = []
             ...
             timp += str(rpiexno) + str(int(hist)) + str(int(nr)) + str(int(decN)) + str(int(decS))
             ...
             timp += fil[len(str(calibrate_time))+1 :len(str(calibrate_time))+4 ] + str(calibrate_time)
             if Cang < 0:
                Canga = -Cang
                timp += "9"
             else:
                Canga = Cang
                timp += "0"
             timp += fil[len(str(Canga))+1        :len(str(Canga))+4        ] + str(Canga)
             timp += fil[len(str(exposure))+1     :len(str(exposure))+4     ] + str(exposure)
             timp += fil[len(str(gain))+1         :len(str(gain))+4         ] + str(gain)
             timp += fil[len(str(gamma))+1        :len(str(gamma))+4        ] + str(gamma)
             timp += fil[len(str(Auto_Gain))+1    :len(str(Auto_Gain))+4    ] + str(Auto_Gain)
             timp += fil[len(str(sharpness))+1    :len(str(sharpness))+4    ] + str(sharpness)
             timp += fil[len(str(saturation))+1   :len(str(saturation))+4   ] + str(saturation)
             timp += fil[len(str(int(color_temp/10)))+1   :len(str(int(color_temp/10)))+4   ] + str(int(color_temp/10))
             #print (len(timp),timp)
             if len(timp) == 181:
             ...
             if os.path.exists(deffile + ".txt"):
                ...
                if len(inputx) > 168:
                   ...
                   gamma = int(inputx[166:169])
                if len(inputx) > 171:
                   Auto_Gain = int(inputx[169:172])
                if len(inputx) > 180:
                   sharpness  = int(inputx[172:175])
                   saturation = int(inputx[175:178])
                   color_temp = int(inputx[178:181]) * 10
   ...
   if change :
      ...
      if oldInterval != Interval:
         keys(0,str(int(Interval)), fs, 3, b1x, bw, 5, b1y, bh, 5, 3, 1)
      if oldrpired != rpired :
         if use_Pi_Cam == 1 or (not use_Pi_Cam and Webcam == 0):
            keys(0,str(rpiredx), fs, 3, b1x, bw, 1, b1y, bh, 5, 3, 1)
         if not use_Pi_Cam  and Webcam == 0 and camera_connected:
            rpistr = "v4l2-ctl -c red_balance=" + str(rpired) + " -d " + str(dve)
            p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         oldrpired = rpired
