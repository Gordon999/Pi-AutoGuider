...
Webcam =              1 # 0 = Philips, 1 = Logitech, 2 = Haupagge USB-Live2
...
if not use_Pi_Cam and camera_connected and Webcam:
...
# start webcam
elif camera_connected and not use_Pi_Cam:
   pygame.camera.init()
...
# take picture
  # webcam
   if camera_connected and not use_Pi_Cam:
      ...
      if not Webcam and zoom > 1 : # Philips
...
# Display
   ...
   # calibrate RA
      ...
      if use_Pi_Cam and camera_connected:
         ...
         if not rpiISO:
            keys(0,'auto',               fs,   3,        b2x,         bw,   1,     b2y, bh, 6, 3, 1)
      if not use_Pi_Cam and camera_connected and not Webcam:
         button(b2x, 1, b2y, 6, bw, 2, bh, 0,1)
         if Webcam == 0:
            keys(0,"Gamma",              fs,   6,        b2x,         bw,   0,     b2y, bh, 6, 0, 1)
            keys(0," -",                 fs,   6,        b2x,         bw,   0,     b2y, bh, 6, 4, 1)
...
# read mouse or Keyboard
   ...
   if buttonx[0] != 0 or mousepress == 1 and store == 0 and calibrate == 0:
      ...
      if z == 161  and zoom > Image_window:
         ...
         if (use_Pi_Cam or (not use_Pi_Cam and not Webcam)) and camera_connected:
            rpired -=1
         else:
            saturation -=1
         if use_Pi_Cam and camera_connected:
            rpiredx = Decimal(rpired)/Decimal(100)
         elif not use_Pi_Cam and camera_connected and not Webcam:
            rpiredx = max(rpired, usb_min_rx)
            rpistr = "v4l2-ctl -c red_balance=" + str(rpiredx) + " -d " + str(dve)
            p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         elif not use_Pi_Cam and camera_connected and Webcam:
         ...
         change = 1
      elif z == 64 and not use_Pi_Cam and camera_connected and not Auto_Gain:
         gain -=1
         gain = max(gain,usb_min_gn)
         change = 1
      elif z == 74 and not use_Pi_Cam and camera_connected and not Auto_Gain:
         gain +=1
         gain = min(gain,usb_max_gn)
         change = 1
      elif z == 65 and not use_Pi_Cam and camera_connected and esc1 == 0 and not Webcam:
         gamma -=1
         gamma = max(gamma,usb_min_ga)
         change = 1
      elif z == 75 and not use_Pi_Cam and camera_connected and esc1 == 0 and not Webcam:
         gamma +=1
         gamma = min(gamma,usb_max_ga)
         change = 1
      elif z == 65 and not use_Pi_Cam and camera_connected and esc1 == 0 and Webcam:
         sharpness -=1
         sharpness = max(sharpness,usb_min_sh)
         change = 1
      elif z == 75 and not use_Pi_Cam and camera_connected and esc1 == 0 and Webcam:
         sharpness +=1
       ...
       if event.type == QUIT:
          ...
          if z == 115 or kz == 304 or kz == 303:
            ...
            change = 1
             ...
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             change = 1
          elif z == 65 and not use_Pi_Cam and camera_connected and esc1 == 0 and press == 0 and not Webcam:
             gamma -=1
             gamma = max(gamma,usb_min_ga)
             rpistr = "v4l2-ctl -c gamma=" + str(gamma) + " -d " + str(dve)
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             change = 1
          elif z == 75 and not use_Pi_Cam and camera_connected and esc1 == 0 and press == 0 and not Webcam:
             gamma +=1
             gamma = min(gamma,usb_max_ga)
             rpistr = "v4l2-ctl -c gamma=" + str(gamma) + " -d " + str(dve)
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             change = 1
          elif z == 65 and not use_Pi_Cam and camera_connected and esc1 == 0 and press == 0 and Webcam:
             sharpness -=1
             sharpness = max(sharpness,usb_min_sh)
             rpistr = "v4l2-ctl -c sharpness=" + str(sharpness) + " -d " + str(dve)
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             change = 1
          elif z == 75 and not use_Pi_Cam and camera_connected and esc1 == 0 and press == 0 and Webcam:
             sharpness +=1
             sharpness = min(sharpness,usb_max_sh)
             rpistr = "v4l2-ctl -c sharpness=" + str(sharpness) + " -d " + str(dve)
             p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             ...
             if use_Pi_Cam and camera_connected:
                rpiredx = Decimal(rpired)/Decimal(100)
             elif not use_Pi_Cam and camera_connected and press == 0 and not Webcam:
                rpiredx = max(rpired, usb_min_rx)
                rpistr = "v4l2-ctl -c red_balance=" + str(rpiredx) + " -d " + str(dve)
                p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             elif not use_Pi_Cam and camera_connected and press == 0 and Webcam:
                saturation = max(saturation, usb_min_sa)
                rpistr = "v4l2-ctl -c saturation=" + str(saturation) + " -d " + str(dve)
                p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
             restart = 1
   ...
   if change :
      ...
      if oldrpired != rpired :
         ...
         if not use_Pi_Cam  and not Webcam and camera_connected:
            rpistr = "v4l2-ctl -c red_balance=" + str(rpired) + " -d " + str(dve)
            p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         oldrpired = rpired
      ...
      if oldsaturation != saturation :
         if not use_Pi_Cam and Webcam and camera_connected:
      ...
      if oldexposure != exposure and not use_Pi_Cam:
         ...
         keys(0,str(exposure), fs,       3,        b2x,         bw,   1,     b2y, bh, 4, 3, 1)
         if not Webcam and camera_connected:
            rpistr = "v4l2-ctl -c exposure=" + str(exposure) + " -d " + str(dve)
         else:
            rpistr = "v4l2-ctl -c exposure_absolute=" + str(exposure) + " -d " + str(dve)
         p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
      ...
      if oldgain != gain and not use_Pi_Cam and camera_connected:
         ...
         p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         oldgain = gain
      if oldgamma != gamma and not use_Pi_Cam and not Webcam and camera_connected:
         keys(0,str(gamma),    fs,       3,        b2x,         bw,   1,     b2y, bh, 6, 3, 1)
         rpistr = "v4l2-ctl -c gamma=" + str(gamma) + " -d " + str(dve)
         p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         oldgamma = gamma
      if oldsharpness != sharpness and not use_Pi_Cam and Webcam and camera_connected:
         keys(0,str(sharpness),    fs,       3,        b2x,         bw,   1,     b2y, bh, 6, 3, 1)
         rpistr = "v4l2-ctl -c sharpness=" + str(sharpness) + " -d " + str(dve)
         p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
