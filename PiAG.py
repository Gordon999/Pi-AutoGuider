...
calibrate_time =    16000 # sets time telescope moved for calibrate, will be varied dependent on zoom level set
...
keys(0,"RELOAD",               fs-2,     7,        b3x+bw/10,    bw,   0,     b3y, bh, 5, 4, 0)
keys(0,"SAVE",                 fs-2,     7,        b3x+bw/4,    bw,   3,     b3y, bh, 5, 4, 0)
if Night:
...
#display graph if enabled
   ...
   if calibrate == 1 and cal_count < 2:
      if cal_count == 0:
         ...
         while zt <= zoom:
            ...
            zt += 1
         cal_time = int(calibrate_time / scale_t)
      pygame.draw.rect(windowSurfaceObj, blackColor, Rect(b3x+(bw * 1.62),b3y + bh*4.65, bw*2 - 8, bh/2.4))
      pygame.display.update()
      if cal_count == 0:
         oauto_g = auto_g
         auto_g = 0
      ...
      if cal_count == 1:
         tim = cal_time
         oVcorrt = int(Vcorrt[4:9])
         oHcorrt = int(Hcorrt[4:9])
         oacorrect = acorrect
         obcorrect = bcorrect
      if Cwindow == 1:
         mmq, mask, change = MaskChange()
   ...
   elif calibrate == 1 and cal_count == 2:
      ccorrect = acorrect - oacorrect
      dcorrect = bcorrect - obcorrect
      if (abs(ccorrect)) > 20 or (abs(dcorrect)) > 20:
         if dcorrect == 0:
            dcorrect = 0.01
         ...
         if ccorrect >= 0 and dcorrect < 0:
            Dang = int(0 - (90 + Dang))
         keys(0,str(Dang),fs-2, 2,b3x+(bw * 1.7),bw,0,b3y, bh, 5, 4, 2)
         nscale = int(tim/(math.sqrt(((ccorrect/100)*(ccorrect/100))+((dcorrect/100)*(dcorrect/100)))))
         if nscale < 0:
            nscale = 0 - nscale
         sscale = nscale
      cal_count +=1
   ...
   elif calibrate == 1 and (cal_count == 3 or cal_count == 4):
      if cal_count == 3:
         tim = int(cal_time/20)
      else:
         tim = cal_time
   ...
   elif calibrate == 1 and cal_count == 5:
      offset3 = offset3 + int(bcorrect/100)
      offset4 = offset4 - int(acorrect/100)
      cal_count +=1
      
   # calibrate RA   
   elif calibrate == 1 and cal_count < 8:
      time.sleep(1)
      keys(0," CALIB  RA",                fs,   3,        b3x+bw/5,    bw,   0,     b3y, bh, 5, 0, 1)
      if cal_count == 6:
         tim = int(cal_time/20)
      if cal_count == 7:
         tim = cal_time
      ...
      if (abs(ccorrect)) > 20 or (abs(dcorrect)) > 20:
         ...
         if Rang < -180:
            Rang = Rang + 360
         keys(0,str(Rang),fs-2, 3,b3x+(bw * 2.48),bw,0,b3y, bh, 5, 4, 2)

         if abs(Dang) > 100 and abs(Rang) < 80 :
            nsi += 1
            if nsi > 1:
               nsi = 0
            if Dang > 100:
            ...
            if Dang < -100:
               Dang = -180 - Dang
            pygame.draw.rect(windowSurfaceObj, blackColor, Rect(b3x+(bw * 1.62),b3y + bh*4.65, bw*1 - 8, bh/2.4))
            pygame.display.update()
            keys(0,str(Dang),fs-2, 3,b3x+(bw * 1.7),bw,0,b3y, bh, 5, 4, 2)
         if abs(Rang) > 100 and abs(Dang) < 80 :
            ewi +=1
            if ewi > 1:
               ewi = 0
            if Rang > 100:
            ...
            if Rang < -100:
               Rang = -180 - Rang
            pygame.draw.rect(windowSurfaceObj, blackColor, Rect(b3x+(bw * 2.64),b3y + bh*4.65, bw*1 - 8, bh/2.4))
            pygame.display.update()
            keys(0,str(Rang),fs-2, 3,b3x+(bw * 2.64),bw,0,b3y, bh, 5, 4, 2)
         if abs(Rang) > 100 and abs(Dang) > 100 :
            ewi +=1
            if ewi > 1:
               ewi = 0
            nsi +=1
         ...
         if abs(Dang - Rang) < 20:
            ...
            pygame.draw.rect(windowSurfaceObj, blackColor, Rect(b3x+(bw * 1.62),b3y + bh*4.65, bw*2 - 8, bh/2.4))
            pygame.display.update()
            keys(0,str(Aang),fs-2, 3,b3x+(bw * 2),bw,0,b3y, bh, 5, 4, 2)
      ...
      keys(0,"TELESCOPE",                fs,   1,        b3x+bw/5,    bw,   0,     b3y, bh, 5, 0, 1)
      esc1 = 0
      keys(0,"Esc",fs,5,b3x,bw,2,b3y, bh, 2, 2, 0)
      keys(0,"cen",                      fs,       7,        b3x,         bw,   1,     b3y, bh, 3, 0, 0)
      keys(0," tre",                     fs,       7,        b3x,         bw,   1,     b3y, bh, 3, 2, 0)
...
# read mouse or Keyboard
   ...
   elif buttonx[0] != 0 or mousepress == 1 and store == 0 and calibrate == 0:
      press = 1
      mousepress = 0
      pos = pygame.mouse.get_pos()
      mousex = pos[0]
   ...
   for event in pygame.event.get():
       if event.type == QUIT:
          ...
          if esc1 > 0 and z != 132 and z != 33 and z != 43 and z != 53 and z != 141 and z != 143 and z != 154 and kz != K_ESCAPE and esc2 != 1:
             esc1 = 0
             keys(0,"Esc",fs,5,b3x,bw,2,b3y, bh, 2, 2, 0)
          ...
          #elif esc1 > 0 and (z == 131 or z == 133 or z == 122 or z == 142):
          elif esc1 > 0 and z == 132:
                calibrate = 1
                cal_count = 0
                keys(0," CALIB DEC",fs,2,b3x+bw/5,bw,   0,     b3y, bh, 5, 0, 1)
                change = 1
