   ...
   if buttonx[0] != 0 or mousepress == 1 and store == 0 and calibrate == 0:
      ...
      if z == 4:         
         ...
         if not use_Pi_Cam and camera_connected:
            ...
            rpico = min(rpico, 100)
         change = 1
      elif z == 63 and not use_Pi_Cam and camera_connected:
         exposure -=1
         exposure = max(exposure,usb_min_ex)
         rpistr = "v4l2-ctl -c exposure=" + str(exposure) + " -d " + str(dve)
         p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         change = 1
      elif z == 73 and not use_Pi_Cam and camera_connected:
         exposure +=1
         exposure = min(exposure,usb_max_ex)
         rpistr = "v4l2-ctl -c exposure=" + str(exposure) + " -d " + str(dve)
         p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         change = 1
      elif z == 64 and not use_Pi_Cam and camera_connected:
         gain -=1
         gain = max(gain,usb_min_gn)
         rpistr = "v4l2-ctl -c gain=" + str(gain) + " -d " + str(dve)
         p = subprocess.Popen(rpistr, shell=True, preexec_fn=os.setsid)
         change = 1
      elif z == 74 and not use_Pi_Cam and camera_connected:
         gain +=1
