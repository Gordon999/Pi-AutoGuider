...
use_Pi_Cam =          0 # if using the RPi camera set use_Pi_Cam = 1 , if set to 0 the program will use a USB camera.
...
brightness   = 64  #sets USB camera initial brightness
usb_min_br   = 0
usb_max_br   = 127
contrast     = 32  #sets USB camera initial contrast
usb_min_co   = 0
usb_max_co   = 63
exposure     = 200 #sets USB camera initial exposure
usb_min_ex   = 0
usb_max_ex   = 255
gain         = 41  #sets USB camera initial gain
usb_min_gn   = 0
usb_max_gn   = 63
gamma        = 16  #sets USB camera initial gamma
usb_min_ga   = 0
usb_max_ga   = 31
red_balance  = 96  #sets USB camera initial red balance
usb_min_rx   = 0
usb_max_rx   = 255
blue_balance = 96  #sets USB camera initial blue balance
usb_min_bx   = 0
usb_max_bx   = 255
  ...
  # webcam
   if camera_connected and not use_Pi_Cam:
      ...
      if zoom > 1 :
         ...
         #pygame.image.save(cropped, 'cropped2.jpg')
...
#display graph if enabled
   ...
   if not fc or use_Pi_Cam or not camera_connected or thres:
      ...
      if menu == 0:
         ...
         if calibrate == 1:
            ...
            pygame.draw.line(windowSurfaceObj,bredColor, (w2 + (crop/3 + 5),h2 - (crop/3 + 5)), (w2 - (crop/3 + 5),h2 + (crop/3 + 5)))
            if cal_count == 5 or cal_count == 10:
   ...
   if calibrate == 1 and cal_count == 0:
   ...
   elif calibrate == 1 and cal_count == 1:
      offset3 = offset3 + int(bcorrect/100)
      offset4 = offset4 - int(acorrect/100)
      cal_count +=1
      
   elif calibrate == 1 and cal_count < 4:
      if ttot < 6:
         calibrate = 0
         cal_count = -1
         crop = ocrop
      ...
      if calibrate == 1 and cal_count == 2:
         keys(0,str(int(threshold)),fs,3,b1x,bw,5,b1y, bh, 4, 3, 1)
         zt = 0
         scale_t = 1
         while zt <= zoom:
         ...
         pygame.display.update()
      if calibrate == 1 and cal_count == 3:
         tim = cal_time
         oacorrect = acorrect
         obcorrect = bcorrect
   ...
   elif calibrate == 1 and cal_count < 7:
      if cal_count == 5:
         tim = int(cal_time/5)
      else:
         tim = cal_time
   ...
   elif calibrate == 1 and cal_count < 10:
       ...
       if event.type == QUIT:
          ...
          if z == 115 or kz == 304 or kz == 303:
            ...
            change = 1
             ...
             if mnr > 2:
                mnr = 0
             keys(0,"WINDOW",                   fs,       mnr+1,        b3x+bw/6,    bw,   3,     b3y-2, bh, 5, 0, 1)
             esc1 = 0
             keys(0,"Esc", fs, 5, b3x, bw, 2, b3y, bh, 2, 2, 1)
             keys(0,"threshold", fs-1,6,b1x,bw,4,b1y, bh, 4, 0, 1)
             keys(0,str(int(threshold)),fs,3,b1x,bw,5,b1y, bh, 4, 3, 1)
             ...
             if use_Pi_Cam and camera_connected:
                os.killpg(p.pid, signal.SIGTERM)
             if (not use_Pi_Cam and zoom <= usb_max_res + 1) or (use_Pi_Cam and Pi_Cam == 2 and zoom < 9) or (use_Pi_Cam and Pi_Cam == 1 and zoom < 7):
                zoom += 1
                if nr > 0:
                   mxq = []
                   mxp = []
